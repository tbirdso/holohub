...
void UxBoundingBoxRenderer::drawEdge(UxEdge& state, Eigen::Vector3f p0, Eigen::Vector3f p1) {
  if (state.range > 0) {
    ...
    Eigen::Vector3f s0 = focal_point - 0.5 * segment_length * d;
    Eigen::Vector3f s1 = focal_point + 0.5 * segment_length * d;
...
void UxBoundingBoxRenderer::render(UxBoundingBox& box) {
  //
  //        |   -z
  //        y   /
  //        |  /
  ...
  drawCorner(box.corners[0], vertices[0], dx, dy, -dz);
  drawCorner(box.corners[1], vertices[1], -dx, dy, -dz);
  drawCorner(box.corners[2], vertices[2], -dx, dy, dz);
  drawCorner(box.corners[3], vertices[3], dx, dy, dz);
  drawCorner(box.corners[4], vertices[4], dx, -dy, -dz);
  drawCorner(box.corners[5], vertices[5], -dx, -dy, -dz);
  drawCorner(box.corners[6], vertices[6], -dx, -dy, dz);
  drawCorner(box.corners[7], vertices[7], dx, -dy, dz);
  ...
  drawEdge(box.edges[0], vertices[0], vertices[1]);
  drawEdge(box.edges[1], vertices[1], vertices[2]);
  drawEdge(box.edges[2], vertices[2], vertices[3]);
  drawEdge(box.edges[3], vertices[3], vertices[0]);

  drawEdge(box.edges[4], vertices[0], vertices[4]);
  drawEdge(box.edges[5], vertices[1], vertices[5]);
  drawEdge(box.edges[6], vertices[2], vertices[6]);
  drawEdge(box.edges[7], vertices[3], vertices[7]);

  drawEdge(box.edges[8], vertices[4], vertices[5]);
  drawEdge(box.edges[9], vertices[5], vertices[6]);
  drawEdge(box.edges[10], vertices[6], vertices[7]);
  drawEdge(box.edges[11], vertices[7], vertices[4]);
