...
void XrSession::initialize() {
  ...
  if (!transition_state(xr::SessionState::Ready, 3000ms)) {
    throw std::runtime_error("Failed to begin session, transitioning to ready session state timed out.");
  }
...
const XrEventDataBaseHeader* XrSession::try_read_next_event(xr::EventDataBuffer& eventDataBuffer) {
    ...
    XrEventDataBaseHeader* baseHeader = reinterpret_cast<XrEventDataBaseHeader*>(eventDataBuffer.get());
    *baseHeader = {XR_TYPE_EVENT_DATA_BUFFER};
    ...
    throw std::runtime_error(std::string("xrPollEvent Failed!, error code: ") + std::to_string((std::int32_t)xr));
}

bool XrSession::transition_state(const xr::SessionState new_state, const std::chrono::milliseconds timeout_ms) {
...
void XrSession::process_session_state_changed(const XrEventDataSessionStateChanged& stateChangedEvent, XrSession::PollResult& pollState) {
  const xr::SessionState oldState = session_state_;
  session_state_ = static_cast<xr::SessionState>(stateChangedEvent.state);

  //HOLOSCAN_LOG_DEBUG("XrEventDataSessionStateChanged: state %d->%d session=%lld time=%lld", oldState,
  //                                  session_state_, stateChangedEvent.session, stateChangedEvent.time);

  if ((stateChangedEvent.session != XR_NULL_HANDLE) && (stateChangedEvent.session != session_.getRawHandle())) {
