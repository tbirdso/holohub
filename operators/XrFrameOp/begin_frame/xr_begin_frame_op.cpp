...
void XrBeginFrameOp::compute(InputContext& input, OutputContext& output,
  ...
  if (pollResult.exit_render_loop)
    throw std::runtime_error("XrInstance/Session is or pending to be lost, or is currently exiting");
  ...
  for (std::size_t hand_idx = 0; hand_idx < aim_space_.size(); ++hand_idx) {
    const auto aim_location = aim_space_[hand_idx]->locateSpace(space, frame.state.predictedDisplayTime);
    ...
    const auto grip_location = grip_space_[hand_idx]->locateSpace(space, frame.state.predictedDisplayTime);
